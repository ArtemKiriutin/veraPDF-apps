...
class CheckerPanel extends JPanel {
    ...
    private class PanelDropTarget implements DropTargetListener {
        ...
        private void checkTransferType(DropTargetDragEvent dropTargetDragEvent) {
            // Only accept a flavor that returns a Component
            acceptableType = false;
            DataFlavor[] fl = dropTargetDragEvent.getTransferable().getTransferDataFlavors();
            for (int i = 0; i < fl.length; ++i) {
                if (fl[i].isFlavorJavaFileListType()) {
